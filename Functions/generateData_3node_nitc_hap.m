...
% simulations of GRN output from a haploid 3-gene model {A, A', B} with
% nonsense-induced transcriptional compensation. It is based on the
% symmetric network wrapper function generateData.m by Lea Schuh and Yogesh
% Goyal, 2020.
% Ian Mellis, 2021
...
%%
function generateData_3node_nitc_hap(nruns,n_species,maxgillespie,gen,init,data,type)
...
for iname = 1:10
    ...
    set_onbasal_aprime{iname} = sprintf('onbasal_aprime%d', iname);
    set_onbasal_b{iname} = sprintf('onbasal_b%d', iname);
    set_ondep_prime = sprintf('ondep_prime', iname);
    set_nitc = sprintf('nitc%d', iname);
